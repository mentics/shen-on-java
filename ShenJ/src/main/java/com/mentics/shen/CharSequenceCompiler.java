 ...
 * stringContaininSourceForNewClass,
...
public class CharSequenceCompiler {
     ...
     * 
     * @param loader the application ClassLoader. The compiler will look through to this // class loader for dependent
     *            classes
     * @param options The compiler options (such as "-target" "1.5"). See the usage for javac
     * @throws IllegalStateException if the Java compiler cannot be loaded.
     */
    public CharSequenceCompiler(JavaFileObjectSource loader, Iterable<String> options) {
     ...
     * <p>
     * Thread safety: this method is thread safe if the <var>javaSource</var> and <var>diagnosticsList</var> are
     * isolated to this thread.
     * 
     * @param qualifiedClassName The fully qualified class name.
     * @param javaSource Complete java source, including a package statement and a class, interface, or annotation
     *            declaration.
     * @param diagnosticsList Any diagnostics generated by compiling the source are added to this collector.
     * @param types zero or more Class objects representing classes or interfaces that the resulting class must be
     *            assignable (castable) to.
     * @return a Class which is generated by compiling the source
     * @throws CharSequenceCompilerException if the source cannot be compiled - for example, if it contains syntax or
     *             semantic errors or if dependent classes cannot be found.
     * @throws ClassCastException if the generated class is not assignable to all the optional <var>types</var>.
     */
    public synchronized Class compile(final String qualifiedClassName, final CharSequence javaSource,
     ...
     * <p>
     * Thread safety: this method is thread safe if the <var>classes</var> and <var>diagnosticsList</var> are isolated
     * to this thread.
     * 
     * @param classes A Map whose keys are qualified class names and whose values are the Java source strings containing
     *            the definition of the class. A map value may be null, indicating that compiled class is expected,
     *            although no source exists for it (it may be a non-public class contained in one of the other strings.)
    ...
    public synchronized Map<String, byte[]> compileToBytes(final Map<String, CharSequence> classes,
            final DiagnosticCollector<JavaFileObject> diagnosticsList) throws CharSequenceCompilerException {
        List<JavaFileObject> sources = new ArrayList<JavaFileObject>();
        for (Entry<String, CharSequence> entry : classes.entrySet()) {
            String qualifiedClassName = entry.getKey();
        ...
        // Get a CompliationTask from the compiler and compile the sources
        final CompilationTask task = compiler.getTask(null, javaFileManager, diagnosticsList, options, null, sources);
        final Boolean result = task.call();

        Map<String, byte[]> compiled = new HashMap<>();
        if (result) {
            for (JavaFileObject fo : classLoader.compiledFiles()) {
                // TODO: fo.getName right name?
                compiled.put(fo.getName(), ((JavaFileObjectImpl) fo).getByteCode());
            }
        }
        return compiled;
    }
     ...
     * 
     * @param types zero of more classes or interfaces that the <var>newClass</var> must be castable to.
     ...
     */
    private Class castable(Class newClass, Class<?>... types) throws ClassCastException {
...
final class FileManagerImpl extends ForwardingJavaFileManager<JavaFileManager> {
     ...
     * 
     * @param fileManager another FileManager that this instance delegates to for additional source.
     * @param classLoader a ClassLoader which contains dependent classes that the compiled classes will require when
     *            compiling them.
     */
    ...
    public Iterable<JavaFileObject> list(Location location, String packageName, Set<Kind> kinds, boolean recurse)
        ...
        if (location == StandardLocation.CLASS_PATH && kinds.contains(JavaFileObject.Kind.CLASS)) {
            ...
            files.addAll(classLoader.files(packageName));
...
final class JavaFileObjectImpl extends SimpleJavaFileObject {
    ...
    JavaFileObjectImpl(final String name, final Kind kind, byte[] bytes) {
        super(CharSequenceCompiler.toURI(name), kind);
        source = null;
...
final class ClassLoaderImpl extends ClassLoader {
    ...
    private JavaFileObjectSource delegate;


    ClassLoaderImpl(final JavaFileObjectSource delegate) {
        super();
     ...
     */
    Collection<JavaFileObject> compiledFiles() {
        return Collections.unmodifiableCollection(classes.values());
    }
    
    Collection<JavaFileObject> files(String packageName) {
        return delegate.files(packageName);
    }
