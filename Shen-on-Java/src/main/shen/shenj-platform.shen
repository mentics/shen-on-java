    ...
    (let Result (parsed-kl-to-java Parsed)
      (let Unit (to-java-unit (fst Result) (second Result))
        (let Classname (fst Unit)
             ...
             (write-source (@s (value *src-dir*) (string-replace "." "/" Classname) ".java") Class-unit-content)))))
...
(define to-java-unit { string -> string -> (@p string string) \* class-name, contents *\ }
  Function-content Result-expression ->
    (let Info (if (= Result-expression "")
         ...
         Class-simple-name (second Info)
         Package-name (fst Info)
      (@p (@s Package-name "." Class-simple-name) (java-class-file Class-simple-name Package-name Function-content))))
...
(define parse-single-kl
  String -> (hd (read-from-string String)))
