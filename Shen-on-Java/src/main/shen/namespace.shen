		    ...
		    (let Aliased (namespace/process-alias Struct Symbol)
		      (if (= Aliased Symbol)
		          (let Suffixed (namespace/process-suffixes Struct Symbol)
		            (if (= Suffixed Symbol)
                    (let Check-java (if (element? shenj.dot/ (fourth Struct))
                         ...
                         Java-call (if (is-java-call Check-java)
                                       (let Call-info (parse-java-call-symbol Check-java)
                                         (if (= Call-info ())
                                              ()
                                              (let Callstr (second Call-info)
                                                   Extra-token (or (= static-method (fst Call-info)) (= static-field (fst Call-info)) (= instance-method (fst Call-info)))
                                                   Search (if Extra-token
                                                              (shenj.dot/%com.mentics.util.StringUtil.removeLastToken "." Callstr)
                                                              Callstr)
                                                   Result (namespace/look-in-imports Struct (intern Search))
                                                (if (= Result ())
                                                    Check-java
                                                    (let Sym (concat shenj.dot/ Result)
                                                         Sym' (if Extra-token (intern (@s (str Sym) "." (shenj.dot/%com.mentics.util.StringUtil.lastToken "." Callstr))) Sym)
                                                      (back-to-dot-notation Call-info Sym'))))))
                                       ())
                      (if (= () Java-call)
